...
const CONFIG = {
  // Cache-busted URL helps iPhones/GitHub Pages pick up updates reliably.
  templateFile: "maintenance_template.pdf?v=2.2",
  ...
  const html = CONFIG.pages.map((p, pageIndex) => {
    ...
    return `
      <section class="card">
        ...
        <div class="hint"><b>If equipment not used today, mark N/A.</b><br/>Up to 4 photos per checklist page (matches the 4 photo boxes on the PDF).</div>
...
function handlePagesChange(e) {
  ...
  if (na) {
    ...
    if (na.checked) {
      ...
      state.pages[pageIndex].additionalComments = "";
    }
